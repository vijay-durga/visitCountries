...
class CountriesList extends Component {
  ...
  onClickVisit = id => {
    ...
    const visitedCountry = visitedCountrieslist.map(each => {
      if (each.id === id) {
  ...
  onRemoveBtn = id => {
    const {visitedCountrieslist} = this.state
    const removedCountry = visitedCountrieslist.map(eachCountry => {
    ...
    this.setState({visitedCountrieslist: removedCountry})
  }
    ...
    return (
      <div className="bg-cont">
        ...
        {updatedList.length < 1 ? (
          <p className="para-1">No Countries Visited Yet</p>
        ) : (
          <ul className="visited-list-cont">
